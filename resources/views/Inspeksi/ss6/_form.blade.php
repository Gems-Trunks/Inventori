      ...
      @foreach ($dataUnit as $name => $label)
         ...
         <div class="col-md-6 mb-3">
            ...
            @if ($name === 'tanggal_inspeksi')
               ...
               <input type="date" name="{{ $name }}" id="{{ $name }}" class="form-control @error($name) is-invalid @enderror"
                  value="{{ old($name, $inspeksi->$name ?? '') }}" required>

            @else

               <input type="text" name="{{ $name }}" id="{{ $name }}" class="form-control @error($name) is-invalid @enderror"
                  value="{{ old($name, $inspeksi->$name ?? '') }}" placeholder="Masukkan {{ strtolower($label) }}" required>
   ...
   <div class="row">
      ...
      @foreach ($kondisiFields as $name => $label)
         ...
         @php
            ...
            $ketersediaanValue = old(
               $name . '_ketersediaan',
               $dataKondisi['ketersediaan'] ?? ''
            );
         ...
         <div class="col-md-6 mb-4">
            ...
            <div class="mb-2">
               ...
               <div class="form-check form-check-inline">
                  ...
                  <input type="radio" class="form-check-input" name="{{ $name }}_ketersediaan" id="{{ $name }}_ada"
                     value="Ada" {{ $ketersediaanValue === 'Ada' ? 'checked' : '' }} required>
               ...
               <div class="form-check form-check-inline">
                  ...
                  <input type="radio" class="form-check-input" name="{{ $name }}_ketersediaan" id="{{ $name }}_tidak_ada"
                     value="Tidak ada" {{ $ketersediaanValue === 'Tidak ada' ? 'checked' : '' }}>
            ...
            <div>
               ...
               <div class="form-check form-check-inline">
                  ...
                  <input type="radio" class="form-check-input" name="{{ $name }}_kondisi" id="{{ $name }}_baik"
                     value="Baik" {{ $kondisiValue === 'Baik' ? 'checked' : '' }} required>
               ...
               <div class="form-check form-check-inline">
                  ...
                  <input type="radio" class="form-check-input" name="{{ $name }}_kondisi" id="{{ $name }}_rusak"
                     value="Rusak" {{ $kondisiValue === 'Rusak' ? 'checked' : '' }}>
...
<div class="mb-4">
   ...
   <textarea name="keterangan" id="keterangan" rows="4" class="form-control @error('keterangan') is-invalid @enderror"
      placeholder="Masukkan keterangan jika ada">{{ old('keterangan', $inspeksi->keterangan ?? '') }}</textarea>
