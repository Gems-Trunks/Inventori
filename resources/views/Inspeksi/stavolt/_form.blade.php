...
<div class="row g-3">
    ...
    <div class="col-md-3">
        <label for="tanggal_inspeksi" class="form-label">Tanggal Inspeksi</label><input id="tanggal_inspeksi"
            type="date" name="tanggal_inspeksi" class="form-control @error('tanggal_inspeksi') is-invalid @enderror"
    ...
    @foreach ([
        'inspektor' => ['Inspektor', 'Nama petugas inspeksi'], 
        'jabatan_inspektor' => ['Jabatan Inspektor', 'Contoh: Teknisi IT'], 
        'diketahui_oleh' => ['Diketahui Oleh', 'Nama supervisor / atasan']] as $field => [$label, $placeholder])
        <div class="col-md-3"><label for="{{ $field }}" class="form-label">{{ $label }}</label><input
